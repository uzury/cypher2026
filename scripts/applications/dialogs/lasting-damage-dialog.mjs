...
export function promptAddLastingDamageDialog({ actor }) {
  ...
  const dialog = new foundry.applications.api.DialogV2({
    window: { title: game.i18n.localize("CYPHER2026.Damage.Title") }, content,
    buttons: [
      {
        action: "create", label: game.i18n.localize("CYPHER2026.Damage.CreateButton"), icon: "fas fa-plus", default: true,
        callback: async (event, button) => {
          ...
          await actor.createEmbeddedDocuments("Item", [{
            name, type: "equipment", img: "icons/svg/bones.svg",
            system: { isDamage: true, damageType, severity, value: 1, description, archived: false, woundApplied: true }
          }]);
          await actor.update({
