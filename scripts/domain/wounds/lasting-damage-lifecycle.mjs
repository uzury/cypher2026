...
async function releaseLastingWound(item, { keepCurrent = false } = {}) {
  ...
  if (!keepCurrent) {
    updates[`system.wounds.${severity}.current`] = Math.max(lastingCount - 1, Number(wound.current ?? 0) - 1);
  }
...
export function registerLastingDamageLifecycle() {
  Hooks.on("updateItem", async (item, changed, options) => {
    ...
    // The existing Heal action removes current wound first and then archives the item.
